...
class PostEditAction extends Action{
	...
	public function index(){
		...
		{$post = M('Post')->where(array('postname'=>$postname,'editpart'=>'introduction','formal'=>'false'))->find();
		  $type="职位简介";
		}
	    if($typeorgin=="requirement")
		{$post = M('Post')->where(array('postname'=>$postname,'editpart'=>'requirement','formal'=>'false'))->find();
		  $type="职位需求";
		}
		 if($typeorgin=="improvement")
		{$post = M('Post')->where(array('postname'=>$postname,'editpart'=>'improvement','formal'=>'false'))->find();
		$type="个人提升";
		}
		 if($typeorgin=="employment")
		{ $post =M('Post')->where(array('postname'=>$postname,'editpart'=>'employment','formal'=>'false'))->find();
		$type="招聘信息";
		 }
	...
	public function postEdit(){
		...
		*/
		...
		if($_POST['postname'] != null && $_POST['postname'] != ''){
			$save = M('Post')->where(array('editno'=>$_GET['editno']))->save($_POST);
		}
