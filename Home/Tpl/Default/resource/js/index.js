...
function showEditOrAdd(){
	...
	xhr.onreadystatechange = function(){
		if(xhr.readyState == 4){
			if((xhr.status >= 200 && xhr.status < 300) || xhr.status == 304){
				var names_json = xhr.responseText;
				//alert(names_json);
				var nameSS = [];
				var names_js = JSON.parse(names_json);//SyntaxError: JSON.parse: unexpected non-whitespace character after JSON data
				
				//alert(names_js);
				
				if(null != names_js && '' != names_js){
					for(var i = 0; i < names_js.length; i++){
						nameSS[i] = names_js[i].postname;
					}
	...
	xhr.open("post", "./PostExisted", true);
